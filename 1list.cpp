...
public:
	...
	void insert(int ind, Object* new_obj) { 
		// сдвигаем элементы вправо, начиная с ind, и вставляем элемент в получившееся место
		if (ind < k) {
			for (int i = k; i > ind; i--)
				st[i] = st[i - 1];
			st[ind] = new_obj;
...
int main() {
	...
	for (int i = 0; i < 1000; ++i) {
		...
		else ind = rand() % (myStor.get_count()); // индекс элемента, с которым что-то произойдет (кроме случая 2)
		switch (rand_num)
		{
